...
module.exports = (function() {
  ...
  var adapter = {
    ...
    defaults: {
      ...
      // DB Options
      // w: 1, // deprecated
      // wtimeout: 0, // deprecated
      // fsync: false, // deprecated
      journal: false,
      readPreference: null,
      nativeParser: false,
    ...
    teardown: function (conn, cb) {
      ...
      if (conn === null) {
        ...
        // mongo driver 3+ supports mongoclient.close in place of db.close
        ...
        return async.each(clients, function (client, onClosed) {
          ...
          const callbackClose = callbackify(() => client.close())
          callbackClose(onClosed)
        }, cb);
      }
      ...
      // mongo driver 3+ supports mongoclient.close in place of db.close
      var dbClient = connections[conn].connection.client;

      // mongo driver 5+ removes callbacks and is promise based instead
      const callbackClose = callbackify(() => dbClient.close());
      callbackClose(() => {
    ...
    drop: function(connectionName, collectionName, relations, cb) {
      ...
      connectionObject.connection.dropCollection(collectionName, function(err) {
        ...
        if(err && err.codeName === 'NamespaceNotFound') return cb();
